...
- 依赖：仅 Win32 (Winsock) + jansson（**静态编译**），插件自带线程，不阻塞调试器 GUI
- 双架构：同时构建 `x64dbg_mcp.dp64`（x64dbg）与 `x64dbg_mcp.dp32`（x32dbg），
  x64 与 x32 各自使用独立端口（默认 8765 / 回退 8766），可同时运行互不冲突
- 30 个调试工具，覆盖调试流程的完整闭环
...
│   ├── src/
│   │   ├── mcp.h          # 公共接口
│   │   ├── plugin.c       # 插件入口（回调注册、菜单、命令、事件推送）
│   │   ├── mcp_server.c   # HTTP+SSE 传输层与 JSON-RPC 分发
│   │   └── tools.c        # 30 个 MCP 工具实现
│   ├── third_party/       # jansson 2.14 源码（静态编译进插件）
│   ├── test_mcp.py        # 端到端测试脚本（Python 3 标准库）
│   ├── test_flow.py       # 完整调试流程测试（--port/--target 可配）
│   └── examples/          # MCP 客户端配置示例
├── README.md              # 本文件
└── UPDATES_FIXES.md       # 更新与修复记录
```

## 构建

依赖 [WinLibs MinGW-w64](https://winlibs.com/)（须同时包含 x86_64 与 i686 两套工具链），
以及仓库根目录的 `pluginsdk/` 与 `release/`（含 x64/x32 的 DLL，可改用你本地的
x64dbg 安装目录）。
...
> jansson 以源码形式（`third_party/jansson-2.14`）静态编译进插件，
> **不依赖 x64dbg 附带的 jansson.dll**。原因见
> [UPDATES_FIXES.md](UPDATES_FIXES.md)「32 位跨 CRT 堆损坏崩溃」。
...
1. 启动 `x96dbg.exe`（或 x64dbg/x32dbg），插件自动加载并监听
   ...
   - 端口保存在各自的 `x64dbg.ini` / `x32dbg.ini` 的 `[x64dbg-mcp] Port` 中；
...
python test_flow.py                     :: 完整调试流程：加载程序->反汇编/内存/搜索
                                         :: ->断点->运行/暂停->寄存器->清理
python test_flow.py --port 8765         :: x64dbg（默认）
python test_flow.py --port 8766         :: x32dbg（以其实际端口为准）
```
...
从开始开发到现在修复的所有构建、运行与兼容性问题，详见
[UPDATES_FIXES.md](UPDATES_FIXES.md)。
